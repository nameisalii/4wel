#!/usr/bin/env python3
"""
Generate visualization MCAP files with robot body, wheels, and links.
This script creates MCAP files that can be visualized in Foxglove Studio.

Usage:
    python scripts/generate_visualization_mcap.py --mode single
    python scripts/generate_visualization_mcap.py --mode multi
"""
...
def create_visualization_mcap(model_path: str, 
    ...
    """
    Create an MCAP file with full robot visualization (body, wheels, links, targets).
    
    Args:
        model_path: Path to trained model
        output_file: Output MCAP file path
        mode: 'single' or 'multi'
        num_episodes: Number of evaluation episodes to record
        num_robots: Number of robots
        record_every_n_steps: Record visualization every N steps
    """
    print(f"\n{'='*70}")
    ...
    try:
        ...
        print(f"❌ Error initializing MCAP writer: {e}")
        import traceback
        traceback.print_exc()
        return None
    ...
    try:
        for episode in range(num_episodes):
            ...
            try:
                ...
                print(f"⚠️  Warning: Could not record final markers: {e}")
        ...
        if result:
            ...
            print(f"\n📝 This file contains:")
            print(f"   • Robot body markers (rectangular body)")
            print(f"   • Wheel markers with steering angles (4 wheels per robot)")
            print(f"   • Link markers (connecting body center to each wheel)")
            print(f"   • Target markers (green circles)")
            print(f"   • ICR markers (instantaneous center of rotation)")
        ...
        print(f"\n❌ Error during visualization recording: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def main():
    ...
    # Set number of robots for multi-robot mode
    ...
    if result:
        ...
        print(f"   4. Add a '3D' panel and select '/visualization_markers' channel")
        print(f"   5. You'll see robot visualization with body, wheels, links, and targets!")
    else:
